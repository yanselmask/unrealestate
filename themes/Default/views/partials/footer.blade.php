@php
    $quicklinks = RyanChandler\FilamentNavigation\Models\Navigation::fromHandle('quick-links');
    $aboutlinks = RyanChandler\FilamentNavigation\Models\Navigation::fromHandle('about-links');
    $recents = \App\Models\Post::latest()
        ->limit(2)
        ->get();
@endphp
<footer class="footer bg-secondary pt-5">
    <div class="pt-lg-4 container pb-4">
        <!-- Links-->
        <div class="row pb-md-3 pb-lg-4 mb-5">
            <div class="col-lg-6 mb-lg-0 mb-4">
                <div class="d-flex flex-sm-row flex-column justify-content-between mx-n2">
                    <div class="mb-sm-0 mb-4 px-2">
                        <a class="d-inline-block mb-4" href="real-estate-home-v1.html">
                            <img src="{{ site_logo() }}" width="116" alt="{{ setting('site_name') }}" />
                        </a>
                        <ul class="nav flex-column mb-sm-4 mb-2">
                            @if (setting('site_admin_email'))
                                <li class="nav-item mb-2">
                                    <a class="nav-link fw-normal p-0" href="mailto:{{ setting('site_admin_email') }}">
                                        <i
                                            class="fi-mail mt-n1 me-2 align-middle opacity-70"></i>{{ setting('site_admin_email') }}
                                    </a>
                                </li>
                            @endif
                            @if (setting('site_admin_phone'))
                                <li class="nav-item">
                                    <a class="nav-link fw-normal p-0" href="tel:{{ setting('site_admin_phone') }}">
                                        <i class="fi-device-mobile mt-n1 me-2 align-middle opacity-70"></i>
                                        {{ setting('site_admin_phone') }}
                                    </a>
                                </li>
                            @endif
                        </ul>
                        <div class="pt-2">
                            @foreach (setting('site_social_media_links') as $key => $value)
                    ...
                    @if ($quicklinks)
                        <div class="mb-sm-0 mb-4 px-2">
                            <h4 class="h5">{{ $quicklinks->name }}</h4>
                            <ul class="nav flex-column">
                                @foreach ($quicklinks->items as $item)
                                    <li class="nav-item mb-2">
                                        <a class="nav-link fw-normal {{ $item['data']['classes'] }} p-0"
                                            href="{{ $item['data']['url'] }}"
                                            target="{{ $item['data']['target'] }}">{!! $item['data']['icon'] !!}{!! $item['label'] !!}</a>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                    ...
                    @if ($aboutlinks)
                        <div class="px-2">
                            <h4 class="h5">{{ $aboutlinks->name }}</h4>
                            <ul class="nav flex-column">
                                @foreach ($aboutlinks->items as $item)
                                    <li class="nav-item mb-2">
                                        <a class="nav-link fw-normal {{ $item['data']['classes'] }} p-0"
                                            href="{{ $item['data']['url'] }}"
                                            target="{{ $item['data']['target'] }}">{!! $item['data']['icon'] !!}{!! $item['label'] !!}</a>
        ...
        <div class="bg-dark rounded-3">
            <div
                ...
                @if (setting('site_footer_image_download'))
                    <img class="mt-md-n5 me-md-5 flex-shrink-0" src="{{ setting('site_footer_image_download') }}"
                        width="240" alt="{{ setting('site_footer_heading_download') }}" />
                @endif
                <div
                    class="align-self-center d-flex flex-lg-row flex-column align-items-lg-center pt-md-3 ps-xxl-4 text-md-start pt-5 text-center">
                    <div class="me-md-5">
                        <h4 class="text-light">{{ setting('site_footer_heading_download') }}</h4>
                        <p class="mb-lg-0 text-light">
                            {{ setting('site_footer_text_download') }}
                        </p>
                    </div>
                    <div class="flex-shrink-0">
                        @if (setting('site_footer_link_ios'))
                            <a class="btn-market ms-sm-0 me-sm-4 mx-2 mb-3"
                                href="{{ setting('site_footer_link_ios') }}" role="button">
                        ...
                        @endif
                        @if (setting('site_footer_link_android'))
                            <a class="btn-market mb-3" href="{{ setting('site_footer_link_android') }}" role="button">
        ...
        <div class="fs-sm mt-3 pb-2 pt-4 text-center">
            {!! setting('site_footer_copyright') !!}
        </div>
    </div>
</footer>
